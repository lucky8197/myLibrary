...
function checkUser(str){
	if(str == ""){
		...
		document.getElementById("user_message").style = "color:#f40";
		document.getElementById("user_message").innerHTML = "用户名只能由字母（大小写）和数字组成(2秒后自动获得焦点)";
		window.setTimeout("document.getElementById('user').focus()" , 2000);
		userFlag = false;
	}
}
...
function checkPassword(str){
	if(str == ""){
		...
		document.getElementById("pass_message").style = "color:#f40";
		document.getElementById("pass_message").innerHTML = "密码只能由字母或数字组成";	
		passwordFlag = false;
	}
}
/*获得焦点后，显示提示信息*/
